...
export type MulticallContract<ContractType> = {
  [Fn in keyof ContractType["functions"]]: (...args: Parameters<ContractType[Fn]>) => MulticallCall<
...
export class MulticallProvider extends Provider {
   ...
   *  */
  all<A extends MulticallCall>(calls: readonly [A], overrides: CallOverrides): Promise<[A["outputs"]]>
  all<A extends MulticallCall, B extends MulticallCall>(calls: readonly [A, B], overrides: CallOverrides): Promise<[A["outputs"], B["outputs"]]>
  all<A extends MulticallCall, B extends MulticallCall, C extends MulticallCall>(
    calls: readonly [A, B, C],
    overrides: CallOverrides,
  ): Promise<[A["outputs"], B["outputs"], C["outputs"]]>
  all<A extends MulticallCall, B extends MulticallCall, C extends MulticallCall, D extends MulticallCall>(
