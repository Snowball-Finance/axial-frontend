...
export default function SwapInput({
  ...
  return (
    <div className={styles.swapInputContainer}>
      <div
        className={classnames(styles.selectGroup, onSelect && styles.hoverPointer)}
        ...
        <div className={styles.tokenNameContainer}>
          <div className={styles.symbolArrowContainer}>
            <b className={styles.textBoldPurple}>{selectedToken ? selectedToken.symbol : t("chooseToken")}</b>
            &nbsp;
            {onSelect && (
          ...
          {selectedToken && <p className={styles.textMinor}>{selectedToken.name}</p>}
        </div>
      </div>
      <div
        ...
        <p className={styles.textMinor}>≈${commify(formatBNToString(inputValueUSD, 18, 2))}</p>
      </div>
      {isDropdownOpen && (
