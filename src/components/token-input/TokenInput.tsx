...
import { calculatePrice, commify } from "../../libs"
...
function TokenInput({ symbol, icon, max, inputValue, onChange, disabled }: Props): ReactElement {
  ...
  if (poolData.lpTokenPriceUSD != Zero) {
    tokenUSDValue = parseFloat(formatBNToString(poolData.lpTokenPriceUSD, 18, 2))
  } else {
  ...
  function onChangeInput(e: React.ChangeEvent<HTMLInputElement>): void {
    ...
    const isValidPrecision = periodIndex === -1 || e.target.value.length - 1 - periodIndex <= decimals
      ...
      <div className={styles.tokenInputContainer} id="tokenInput">
        ...
        <div className={styles.inputGroup}>
          ...
          <p className={styles.smallText}>
            ≈$
            {commify(formatBNToString(calculatePrice(inputValue, tokenUSDValue), 18, 2))}
          </p>
        </div>
      </div>
    </div>
