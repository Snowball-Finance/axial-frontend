...
import { getFormattedShortTime } from "../../libs/dateTime"
import { getPoolByAddress } from "../../libs/getPoolByAddress"
  ...
  const fetchTxn = useCallback(async () => {
    ...
    await fetch(axial_SUBGRAPH_URL, {
      ...
      .then((result: Response) => {
        ...
        setTransactionList(newTransactionList.sort((a, b) => b.timestamp - a.timestamp))
      })
      .catch(console.error)
  }, [chainId, t, account])
        ...
        {transactionList.length !== 0 ? (
          transactionList.map((txn, index) => (
            <div key={index} className="transactionItem">
              ...
              <a href={getEtherscanLink(txn.transaction, "tx")} target="_blank" rel="noreferrer" className="transactionLink">
                {/* link icon */}
