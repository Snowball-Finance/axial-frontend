...
export default function FarmOverview({ poolData, poolRoute, userShareData, onClickMigrate }: Props): ReactElement | null {
  ...
  const formattedData = {
    ...
    axialPending: formatBNToShortString(userShareData?.masterchefBalance?.pendingTokens.pendingAxial || Zero, 18),
    avaxPending: formatBNToShortString(userShareData?.masterchefBalance?.pendingTokens.pendingBonusToken || Zero, 18),
    ...
    totalapr: Number(poolData.rapr)
      ? (Number(poolData.rapr) + (poolData.apr ? Number(poolData.apr) : 0) + (poolData.extraapr ? Number(poolData.extraapr) : 0)).toFixed(2) + "%"
      : poolData.rapr === 0
      ? "0%"
      : "-",
    ...
    <div
      ...
      <div className="left">
        ...
        </div>
        {
          ...
          (formattedData.tokens.length > 0 || formattedData.ammTokens.length > 0) && (
            <div className="tokens">
              <span>[</span>
              {(formattedData.ammTokens.length > 0 ? formattedData.ammTokens : formattedData.tokens).map(({ symbol, icon }) => (
      ...
      <div className="right">
        ...
        <div className="buttons">
          <Button
            ...
            disabled={userShareData?.masterchefBalance?.pendingTokens.pendingAxial.eq("0x0")}
            kind="secondary">
            {t("claim")}
          </Button>
          ...
          {shouldMigrate ? (
            <Button kind="temporary" onClick={onClickMigrate} disabled={!hasShare}>
              {t("migrate")}
            </Button>
          ) : (
            <Link to={`${poolRoute}/deposit`}>
              <Button size="medium" kind="primary" disabled={poolData?.isPaused || isOutdated}>
                {t("deposit")}
              </Button>
            </Link>
          )}
