...
function Pools(): ReactElement | null {
  ...
  const visiblePoolList = Object.values(POOLS_MAP).filter(
    ({ type, migration, isOutdated }) =>
      (filter === "all" || type === filter || (filter === "outdated" && (migration || isOutdated))) && type !== PoolTypes.LP
  );
      ...
      <div className={styles.content}>
        {visiblePoolList
          ...
          .map(([poolProps, migrationPool]) => (
            <PoolOverview key={poolProps.name} {...poolProps} onClickMigrate={migrationPool ? () => handleClickMigrate() : undefined} />
          ))}
