...
function Deposit({ poolName }: Props): ReactElement | null {
  ...
  const allTokens = useMemo(() => {
    return Array.from(new Set(POOL.poolTokens.concat(POOL.underlyingPoolTokens || [])));
  ...
  }, [shouldDepositWrapped, updateTokenFormState, POOL.poolTokens, POOL.underlyingPoolTokens]);
  ...
  if (underlyingPoolData.lpTokenPriceUSD != Zero) {
    const underlyingTokenUSDValue = parseFloat(formatBNToString(poolData.lpTokenPriceUSD, 18, 2));
    newTokenPricesUSD = {
      ...tokenPricesUSD,
      ...{
  ...
  const metaSwapContract = useMemo(() => {
    if (isMetaSwap && chainId && library) {
      return getContract(POOL.metaSwapAddresses?.[chainId] as string, META_SWAP_ABI, library, account ?? undefined) as MetaSwap;
    }
    return null;
  ...
  useEffect(() => {
    ...
    async function calculateMaxDeposits(): Promise<void> {
      ...
      const tokenInputSum = parseUnits(allTokens.reduce((sum, { symbol }) => sum + (+tokenFormState[symbol].valueRaw || 0), 0).toFixed(18), 18);
      let depositLPTokenAmount;
      if (poolData.totalLocked.gt(0) && tokenInputSum.gt(0)) {
        if (shouldDepositWrapped) {
          depositLPTokenAmount = metaSwapContract
            ? await metaSwapContract.calculateTokenAmount(
                (POOL.underlyingPoolTokens || []).map(({ symbol }) => tokenFormState[symbol].valueSafe),
      ...
      setPriceImpact(calculatePriceImpact(tokenInputSum, depositLPTokenAmount, poolData.virtualPrice));
    }
  ...
  const tokens = (shouldDepositWrapped ? POOL.underlyingPoolTokens || [] : POOL.poolTokens).map(({ symbol, name, icon, decimals }) => ({
    symbol,
    name,
    icon,
  ...
  const exceedsWallet = allTokens.some(({ symbol }) => {
    const exceedsBoolean = (tokenBalances?.[symbol] || Zero).lt(BigNumber.from(tokenFormState[symbol].valueSafe));
  ...
  return (
    <DepositPage
      ...
      onToggleDepositWrapped={() => setShouldDepositWrapped((prevState) => !prevState)}
...
function buildTransactionData(
  ...
  poolTokens.forEach((token) => {
    ...
    const usdPriceBN = parseUnits((tokenPricesUSD?.[symbol] || 0).toFixed(2), 18);
    ...
    from.totalAmount = from.totalAmount.add(shiftBNDecimals(amount, TOTAL_AMOUNT_DECIMALS - decimals));
    from.totalValueUSD = from.totalValueUSD.add(usdPriceBN);
  });

  const lpTokenPriceUSD = poolData?.lpTokenPriceUSD || Zero;
  const toTotalValueUSD = estDepositLPTokenAmount.mul(lpTokenPriceUSD)?.div(BigNumber.from(10).pow(poolLpToken.decimals));
  const to = {
    item: {
      token: poolLpToken,
  ...
  const shareOfPool = poolData?.totalLocked.gt(0)
    ? estDepositLPTokenAmount.mul(BigNumber.from(10).pow(18)).div(estDepositLPTokenAmount.add(poolData?.totalLocked))
