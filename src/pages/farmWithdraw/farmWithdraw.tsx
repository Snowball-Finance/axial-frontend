...
function FarmWithdraw({ poolName }: Props): ReactElement {
  ...
  useEffect(() => {
    ...
    function calculateWithdrawBonus(): void {
      ...
      const tokenInputSum = userShareData.masterchefBalance?.userInfo.amount ?? BigNumber.from("0")
  ...
  }, [poolData, withdrawFormState, swapContract, userShareData, account, POOL.lpToken])
  ...
  if (BigNumber.from(withdrawFormState.tokenInputs[POOL.lpToken.symbol].valueSafe).gt(0)) {
    reviewWithdrawData.withdraw.push({
      name: POOL.lpToken.name,
      value: commify(formatUnits(withdrawFormState.tokenInputs[POOL.lpToken.symbol].valueSafe, POOL.lpToken.decimals)),
      icon: POOL.lpToken.icon,
    })
    if (tokenPricesUSD != null) {
      reviewWithdrawData.rates.push({
        name: POOL.lpToken.name,
        value: formatUnits(withdrawFormState.tokenInputs[POOL.lpToken.symbol].valueSafe, POOL.lpToken.decimals),
        rate: "0",
      })
    }
  }
