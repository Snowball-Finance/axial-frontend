...
function FarmDeposit({ poolName = AXIAL_AS4D_POOL_NAME }: Props): ReactElement | null {
  ...
  const allTokens = useMemo(() => {
    const arr = Array.from(new Set(POOL.poolTokens.concat(POOL.underlyingPoolTokens || [])))
  ...
  }, [shouldDepositWrapped, updateTokenFormState, POOL.poolTokens, POOL.underlyingPoolTokens, POOL.lpToken])
  ...
  }, [poolData, tokenFormState, swapContract, userShareData, account, POOL.poolTokens, POOL.underlyingPoolTokens, shouldDepositWrapped, allTokens])
  ...
  const tokens = [
    {
      ...
      max: formatBNToString(tokenBalances?.[POOL.lpToken.symbol] || Zero, POOL.lpToken.decimals),
