...
import { POOLS_MAP, PoolName, TRANSACTION_TYPES, AXIAL_MASTERCHEF_CONTRACT_ADDRESS } from "../constants"
...
export function useApproveAndWithdraw(
  ...
  const { slippageCustom, slippageSelected, gasPriceSelected, gasCustom, transactionDeadlineCustom, transactionDeadlineSelected, infiniteApproval } =
  ...
  async function approveAndWithdraw(state: ApproveAndWithdrawStateArgument): Promise<void> {
    try {
      if (!account || !chainId) throw new Error("Wallet must be connected")
      const masterchefContract = new ethers.Contract(AXIAL_MASTERCHEF_CONTRACT_ADDRESS[chainId], MASTERCHEF_ABI, library?.getSigner())
      ...
      if (!mastechefWithdraw) {
        const allowanceAmount =
          state.withdrawType === "IMBALANCE" ? addSlippage(state.lpTokenAmountToSpend, slippageSelected, slippageCustom) : state.lpTokenAmountToSpend
        if (!swapContract) return
        await checkAndApproveTokenForTrade(lpTokenContract, swapContract.address, account, allowanceAmount, infiniteApproval, gasPrice, {
          onTransactionError: () => {
        ...
        console.debug(`lpTokenAmountToSpend: ${formatUnits(state.lpTokenAmountToSpend, 18)}`)
        const deadline = Math.round(new Date().getTime() / 1000 + 60 * formatDeadlineToNumber(transactionDeadlineSelected, transactionDeadlineCustom))
        ...
        if (state.withdrawType === "ALL") {
          spendTransaction = await swapContract.removeLiquidity(
            ...
            POOL.poolTokens.map(({ symbol }) =>
              subtractSlippage(BigNumber.from(state.tokenFormState[symbol].valueSafe), slippageSelected, slippageCustom),
            ),
            deadline,
          )
        } else if (state.withdrawType === "IMBALANCE") {
          spendTransaction = await swapContract.removeLiquidityImbalance(
            POOL.poolTokens.map(({ symbol }) => state.tokenFormState[symbol].valueSafe),
            addSlippage(state.lpTokenAmountToSpend, slippageSelected, slippageCustom),
            deadline,
          )
        } else {
          ...
          spendTransaction = await swapContract.removeLiquidityOneToken(
            state.lpTokenAmountToSpend,
            POOL.poolTokens.findIndex(({ symbol }) => symbol === state.withdrawType),
            subtractSlippage(BigNumber.from(state.tokenFormState[state.withdrawType || ""].valueSafe), slippageSelected, slippageCustom),
            deadline,
          )
        }
        ...
        await masterchefContract.withdraw(POOL.lpToken.masterchefId, BigNumber.from(state.tokenFormState[POOL.lpToken.symbol].valueSafe))
      }
      ...
      analytics.trackEvent({
        category: "Withdraw",
        action: "Withdraw",
        name: `symbol:${POOL.lpToken.symbol}-type:${state.withdrawType}-spend:${state.lpTokenAmountToSpend.toNumber()}-valueSafe:${
