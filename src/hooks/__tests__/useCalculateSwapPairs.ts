import { ChainId, Pool, PoolName, PoolTypes, SWAP_TYPES, Token } from "../../constants/index";
...
const createTestPool = (name: string, tokens: Token[], type = PoolTypes.USD): Pool => {
  return {
...
describe("getTradingPairsForToken", () => {
  ...
  const tokensMap = allTokens.reduce((acc, t) => ({ ...acc, [t.symbol]: t }), {} as { [symbol: string]: Token });
  const poolsMap = allPools.reduce((acc, p) => ({ ...acc, [p.name]: p }), {} as { [poolName: string]: Pool });
  ...
  it("Tokens can't swap with themselves", () => {
    const pairs = getTradingPairsForToken(tokensMap, poolsMap, allPools, tokensToPoolsMap, tokenA);
    const selfPair = pairs.find((swapData) => swapData.from.symbol === swapData.to.symbol);
  ...
  it("Synth tokens can swap with any other synth token", () => {
    const pairs = getTradingPairsForToken(tokensMap, poolsMap, allPools, tokensToPoolsMap, tokenE);
  ...
  it("A synth token can swap with a nonSynth token in a synth pool", () => {
    const pairs = getTradingPairsForToken(tokensMap, poolsMap, allPools, tokensToPoolsMap, tokenF);
    expect(tokenF.isSynthetic).toBeTruthy();
  ...
  it("A nonSynth token in a synth pool can swap with another nonSynth token in a different synth pool", () => {
    const pairs = getTradingPairsForToken(tokensMap, poolsMap, allPools, tokensToPoolsMap, tokenC);
    expect(tokenC.isSynthetic).toBeFalsy();
