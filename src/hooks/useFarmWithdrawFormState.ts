...
export default function useWithdrawFormState(poolName: PoolName): [WithdrawFormState, (action: WithdrawFormAction) => void] {
  ...
  const calculateAndUpdateDynamicFields = useCallback(
    debounce((state: WithdrawFormState) => {
      if (userShareData == null || (swapContract == null && POOL.type !== PoolTypes.LP) || account == null) return
      ...
      if (state.withdrawType === ALL) {
        ...
        try {
          if (state.percentage) {
            ...
            const inputCalculatedLPTokenAmount = BigNumber.from(state.tokenInputs[POOL.lpToken.symbol].valueSafe)
    ...
    [userShareData, swapContract, POOL.lpToken, tokenInputStateCreators],
  )
  ...
  const handleUpdateForm = useCallback(
    (action: WithdrawFormAction): void => {
      ...
      setFormState((prevState) => {
        ...
        if (action.fieldName === "tokenInputs") {
          ...
          const activeInputTokens = [POOL.lpToken].filter(({ symbol }) => +newTokenInputs[symbol].valueRaw !== 0)
          let withdrawType
    ...
    [POOL.lpToken, calculateAndUpdateDynamicFields, tokenInputStateCreators, tokenInputsEmptyState, formEmptyState],
  )
