import { NumberInputState, numberInputStateCreator } from "../libs/numberInputState";
...
export default function useWithdrawFormState(poolName: PoolName): [WithdrawFormState, (action: WithdrawFormAction) => void] {
  ...
  const calculateAndUpdateDynamicFields = useCallback(
    debounce(async (state: WithdrawFormState) => {
      if (userShareData == null || swapContract == null || account == null) return;
      ...
      if (state.withdrawType === IMBALANCE) {
        try {
          const inputCalculatedLPTokenAmount = await swapContract.calculateTokenAmount(
            POOL.poolTokens.map(({ symbol }) => state.tokenInputs[symbol].valueSafe),
            false
          );
          nextState = inputCalculatedLPTokenAmount.gt(effectiveUserLPTokenBalance)
            ? {
                error: {
                  field: "tokenInputs",
        ...
        try {
          const tokenAmounts = await swapContract.calculateRemoveLiquidity(effectiveUserLPTokenBalance);
          nextState = {
        ...
        try {
          if (state.percentage) {
            const tokenIndex = POOL.poolTokens.findIndex(({ symbol }) => symbol === state.withdrawType);
            ...
            nextState = {
              ...
              tokenInputs: POOL.poolTokens.reduce(
                (acc, { symbol }, i) => ({
                  ...acc,
                  [symbol]: tokenInputStateCreators[symbol](i === tokenIndex ? tokenAmount : "0")
                }),
                {}
              ),
              error: null
  ...
  const handleUpdateForm = useCallback(
      ...
      setFormState((prevState) => {
        ...
        if (action.fieldName === "tokenInputs") {
          ...
          const newTokenInputs = {
            ...
            [tokenSymbolInput]: tokenInputStateCreators[tokenSymbolInput](valueInput)
          };
          const activeInputTokens = POOL.poolTokens.filter(({ symbol }) => +newTokenInputs[symbol].valueRaw !== 0);
          let withdrawType;
          ...
          const isInputInvalid = isNaN(+action.value) || +action.value < 0 || +action.value > 100;
          nextState = isInputInvalid
            ...
            : {
                withdrawType: prevState.withdrawType === IMBALANCE ? ALL : prevState.withdrawType,
                percentage: action.value,
                error: null
              };
    ...
    [POOL.poolTokens, calculateAndUpdateDynamicFields, tokenInputStateCreators, tokenInputsEmptyState, formEmptyState]
  );
