...
type UpdateTokensStateType = (newState: { [token: string]: string | BigNumber }) => void
type UseTokenFormStateReturnType = [TokensStateType, UpdateTokensStateType]

export function useTokenFormState(tokens: Token[]): UseTokenFormStateReturnType {
  ...
  const tokenInputStateCreators: {
    ...
    () =>
      tokens.reduce(
        (acc, token) => ({
          ...acc,
          [token.symbol]: numberInputStateCreator(token.decimals, BigNumber.from("0")),
        }),
        {},
      ),
    [tokens],
