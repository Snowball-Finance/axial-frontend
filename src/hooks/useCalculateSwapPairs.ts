import { POOLS_MAP, Pool, PoolsMap, SWAP_TYPES, TOKENS_MAP, Token, TokensMap } from "../constants/index";
...
export function useCalculateSwapPairs(): (token?: Token) => SwapData[] {
  ...
  return function calculateSwapPairs(token?: Token): SwapData[] {
    ...
    const swapPairs = getTradingPairsForToken(TOKENS_MAP, POOLS_MAP, poolsSortedByTVL, tokenToPoolsMapSorted, token);
...
function buildSwapSideData(token: Token, pool?: Pool): Required<SwapSide> | SwapSide {
  return {
    symbol: token.symbol,
    poolName: pool?.name,
...
function getTradingPairsForToken(
  ...
  const allTokens = Object.values(tokensMap).filter(({ isLPToken }) => !isLPToken);
  const synthPoolsSet = new Set(poolsSortedByTVL.filter(({ isSynthetic }) => isSynthetic));
  const originTokenPoolsSet = new Set(tokenToPoolsMap[originToken.symbol].map((poolName) => poolsMap[poolName]));
  ...
  allTokens.forEach((token) => {
    ...
    else if (originToken.isSynthetic && !token.isSynthetic && tokenPoolsSynthSet.size > 0) {
      const originPool = [...originTokenPoolsSet][0];
      const destinationPool = [...tokenPoolsSynthSet][0];
      const middleSynth = destinationPool.poolTokens.find(({ isSynthetic }) => isSynthetic);
      if (middleSynth) {
        swapData = {
    ...
    else if (!originToken.isSynthetic && originPoolsSynthSet.size > 0 && !token.isSynthetic && tokenPoolsSynthSet.size > 0) {
      const originPool = [...originPoolsSynthSet][0];
      const destinationPool = [...tokenPoolsSynthSet][0];
      const originSynth = originPool.poolTokens.find(({ isSynthetic }) => isSynthetic);
      const destinationSynth = destinationPool.poolTokens.find(({ isSynthetic }) => isSynthetic);
      if (originSynth && destinationSynth) {
        swapData = {
          type: SWAP_TYPES.TOKEN_TO_TOKEN,
          from: buildSwapSideData(originToken, originPool),
          to: buildSwapSideData(token, destinationPool),
          route: [originToken.symbol, originSynth.symbol, destinationSynth.symbol, token.symbol]
        };
      }
    }

    // Case 6: sPool(TokenA) <> sTokenB
    else if (!originToken.isSynthetic && originPoolsSynthSet.size > 0 && token.isSynthetic) {
      const originPool = [...originPoolsSynthSet][0];
      const destinationPool = [...tokenPoolsSet][0];
      const middleSynth = originPool.poolTokens.find(({ isSynthetic }) => isSynthetic);
      if (middleSynth) {
        swapData = {
