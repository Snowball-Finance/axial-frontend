...
describe("gasBNFromState", () => {
  ...
  it("get gas from standard", () => {
    expect(gasBNFromState(gas, GasPrices.Standard, bn("314"))).toEqual(BigNumber.from("1"));
  });

  it("get gas from fast", () => {
    expect(gasBNFromState(gas, GasPrices.Fast, bn("314"))).toEqual(BigNumber.from("10"));
  });

  it("get gas from instant", () => {
    expect(gasBNFromState(gas, GasPrices.Instant, bn("314"))).toEqual(BigNumber.from("100"));
  });

  it("get gas from custom", () => {
    expect(gasBNFromState(gas, GasPrices.Custom, bn("314"))).toEqual(BigNumber.from("314"));
  });
});
