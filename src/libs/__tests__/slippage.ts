...
describe("_applySlippage", () => {
  ...
  it("Calculates 1% subtracted", () => {
    ...
    expect(_applySlippage(input, Slippages.One, customNumberState)).toEqual(expectedResult);
  });

  it("Calculates 0.1% subtracted", () => {
    const expectedResult = input.mul(999).div(1000);
    expect(_applySlippage(input, Slippages.OneTenth, customNumberState)).toEqual(expectedResult);
  });
  ...
  it("Calculates from custom input subtracted", () => {
    ...
    expect(_applySlippage(input, Slippages.Custom, customNumberState)).toEqual(expectedResult);
  });

  it("Calculates 1% added", () => {
    const expectedResult = input.mul(101).div(100);
    expect(_applySlippage(input, Slippages.One, customNumberState, true)).toEqual(expectedResult);
  });

  it("Calculates 0.1% added", () => {
    const expectedResult = input.mul(1001).div(1000);
    expect(_applySlippage(input, Slippages.OneTenth, customNumberState, true)).toEqual(expectedResult);
  });
  ...
  it("Calculates from custom input added", () => {
    ...
    const expectedResult = input.mul(customNumerator).div(customDenominator);
    expect(_applySlippage(input, Slippages.Custom, customNumberState, true)).toEqual(expectedResult);
  });
});
...
describe("formatSlippageToString", () => {
  ...
  const createNumberInputState = numberInputStateCreator(5, BigNumber.from(10).pow(5).mul(33));

  it("formats slippages custom with custom", () => {
    expect(formatSlippageToString(Slippages.Custom, createNumberInputState("3.14159"))).toBe("3.14159");
  });
});
