...
export default async function checkAndApproveTokenForTrade(
  ...
  gasPrice: BigNumber,
  callbacks: {
    onTransactionStart?: (transaction?: ContractTransaction) => (() => void) | undefined
  ...
  const existingAllowance = await srcTokenContract.allowance(spenderAddress, swapAddress)

  console.debug(`Existing ${tokenName} Allowance: ${existingAllowance.toString()}`)
  ...
  async function approve(amount: BigNumber): Promise<void> {
    try {
      ...
      const approvalTransaction = await srcTokenContract.approve(swapAddress, amount)
