...
export function formatBNToShortString(bn: BigNumber, nativePrecision: number): string {
...
export function formatBNToString(bn: BigNumber, nativePrecison: number, decimalPlaces?: number): string {
...
export function formatBNToPercentString(bn: BigNumber, nativePrecison: number, decimalPlaces = 2): string {
...
export function calculateExchangeRate(amountFrom: BigNumber, tokenPrecisionFrom: number, amountTo: BigNumber, tokenPrecisionTo: number): BigNumber {
...
export function formatDeadlineToNumber(deadlineSelected: Deadlines, deadlineCustom?: string): number {
  let deadline = 20;
...
export function getTokenByAddress(address: string, chainId: ChainId): Token | null {
  return Object.values(TOKENS_MAP).find(({ addresses }) => addresses[chainId] && address.toLowerCase() === addresses[chainId].toLowerCase()) || null;
}

export function calculatePrice(amount: BigNumber | string, tokenPrice = 0, decimals?: number): BigNumber {
  ...
  if (typeof amount === "string") {
    ...
    return amount.mul(parseUnits(tokenPrice.toFixed(2), 18)).div(BigNumber.from(10).pow(decimals));
  }
  return Zero;
}
