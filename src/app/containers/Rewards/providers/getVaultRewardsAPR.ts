import { AxialLPData, ExtraTokens, MasterchefApr, PoolInfos, PoolTypes } from "../types"
...
export async function getVaultRewardAprNow(): Promise<MasterchefApr> {
  ...
  for (const pool of Object.values(pools)) {
    try {
      ...
      console.error(
        `Error fetching Pool Reward APY for Pool: ${pool.address}`,
      )
      APRData = {
        ...APRData,
        [pool.address]: {
