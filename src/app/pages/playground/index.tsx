...
export const Playground = () => {
  ...
  const handleDepositIntoAM3DStablecoinsButtonClicked = () => {
    dispatch(RewardsActions.approveAndDeposit(
      {
        poolName: Pools.AXIAL_AM3D,
        tokenAmounts: {
          [TokenSymbols.MIM]: floatToBN(0.1,tokens[TokenSymbols.MIM].decimals),
          [TokenSymbols.USDCe]: floatToBN(0.1,tokens[TokenSymbols.USDCe].decimals),
    ...
    dispatch(RewardsActions.approveAndDeposit(
      {
        ...
        tokenAmounts: {
          [TokenSymbols.am3dUSD]: floatToBN(0.1,tokens[TokenSymbols.am3dUSD].decimals),
        }
      }
    ))
  }
    const handleWithdrawStableCoins = () => {
    dispatch(RewardsActions.approveAndWithdraw(
      {
        poolName: Pools.AXIAL_AM3D,
        type: WithdrawType.ALL,
        lpTokenAmountToSpend:floatToBN(0.1,tokens[TokenSymbols.MIM].decimals)||BigNumber.from('0'),
        tokenAmounts: {
          [TokenSymbols.MIM]: floatToBN(0.1,tokens[TokenSymbols.MIM].decimals),
          [TokenSymbols.USDCe]: floatToBN(0.1,tokens[TokenSymbols.USDCe].decimals),
          [TokenSymbols.DAI]:floatToBN(0.1,tokens[TokenSymbols.DAI].decimals),
      ...
      <ul>
{
  Object.keys(tokensInQueue).map(tokenSymbol => {
    return <li style={{color:'white'}} key={tokenSymbol}>{tokenSymbol}{tokensInQueue[tokenSymbol].toString()} </li>
  }
  )
}
      </ul>
